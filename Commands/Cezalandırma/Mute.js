...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author} kime cezalandıracağını yazmadın! \`.mute @Lowzy/ID <süre> <sebep>\``).setFooter(ayar.footer)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    if (!args[1]) return message.channel.send(embed.setDescription(`${message.author} chat-mute süresi belirtmedin! \`.mute @Lowzy/ID <süre> <sebep>\``).setFooter(ayar.footer).setFooter(ayar.footer)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    let sure = args[1]
        .replace("s", " Saniye")
        .replace("m", " Dakika")
        .replace("h", " Saat")
